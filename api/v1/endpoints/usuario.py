...
from schemas.usuario_schema import UsuarioSchema, UsuarioSchemaArtigos, UsuarioSchemaCreate, UsuarioSchemaUpdate
...
async def signup(
    ...
    novo_usuario: UsuarioModel = UsuarioModel(**usuario.model_dump(exclude={"senha"}), senha=hash_password(usuario.senha))
    db.add(novo_usuario)
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Erro de integridade ao criar usuário")
    return novo_usuario
...
async def get_usuario(
    ...
    result = await db.execute(query)
    usuario: UsuarioSchemaArtigos = result.scalars().one_or_none()
    if not usuario:
...
# PUT Usuario
@router.put("/{id_usuario}", response_model=UsuarioSchema, status_code=status.HTTP_200_OK)
async def put_usuario(
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Erro de integridade ao atualizar usuário")
